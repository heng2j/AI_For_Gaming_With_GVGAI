...
//		rootNodeMap.put("parent", null);
		...
		while(!queue.isEmpty() && elapsedTimer.remainingTimeMillis() > 2 * avgTime && elapsedTimer.remainingTimeMillis() > worstTime){
		   ...
		   pNodeMap.put("name", currentNode.name);
		   
		   if (currentNode.parent == null) {
			   
			   pNodeMap.put("parent", null);
			   
		   }
		   else {
			   
			   pNodeMap.put("parent", currentNode.parent.name);
			   
		   }
		   
		   pNodeMap.put("parent", currentNode);
